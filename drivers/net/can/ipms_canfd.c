...
static int canfd_device_driver_bittime_configuration(struct net_device *ndev){
	...
	//bittiming_temp=((bt->phase_seg1-2)<<SEG_1_SHIFT)|((bt->phase_seg2-1)<<SEG_2_SHIFT)|((bt->sjw-1)<<SJW_SHIFT)|((bt->brp-1)<<PRESC_SHIFT);
	bittiming_temp=((bt->phase_seg1+bt->prop_seg+1-2)<<SEG_1_SHIFT)|((bt->phase_seg2-1)<<SEG_2_SHIFT)|((bt->sjw-1)<<SJW_SHIFT)|((bt->brp-1)<<PRESC_SHIFT);
	/*The input bittime setting is incorrect, should be correct*/
	if( (((int)(bt->phase_seg1+bt->prop_seg+1)-2)<0) || (((int)(bt->phase_seg2)-1)<0) || (((int)(bt->sjw)-1)<0) || (((int)(bt->brp)-1)<0) ){
	...
	if(priv->can_or_canfd==CANFD_MASK){
		//data_bittiming_temp=((dbt->phase_seg1-2)<<SEG_1_SHIFT)|((dbt->phase_seg2-1)<<SEG_2_SHIFT)|((dbt->sjw-1)<<SJW_SHIFT)|((dbt->brp-1)<<PRESC_SHIFT);
		data_bittiming_temp=((dbt->phase_seg1+dbt->prop_seg+1-2)<<SEG_1_SHIFT)|((dbt->phase_seg2-1)<<SEG_2_SHIFT)|((dbt->sjw-1)<<SJW_SHIFT)|((dbt->brp-1)<<PRESC_SHIFT);
		PDEBUG("test phase_seg1 is %d\n",dbt->phase_seg1);

		//if( ((dbt->phase_seg1-2)<0) || (dbt->phase_seg2-1<0) || (dbt->sjw-1<0) || (dbt->brp-1<0)){
		if( (((int)(dbt->phase_seg1+dbt->prop_seg+1)-2)<0) || (((int)(dbt->phase_seg2)-1)<0) || (((int)(dbt->sjw)-1)<0) || (((int)(dbt->brp)-1)<0) ){
...
static int canfd_driver_start_xmit(struct sk_buff *skb, struct net_device *ndev){
	...
	switch(priv->tx_mode){
		...
		case XMIT_PTB_MODE:
			...
			//PDEBUG( "TPA is %d\n",canfd_ioread8(priv->reg_base+CANFD_TCMD_OFFSET)&CAN_FD_SET_TPA_MASK>>3);
			//netif_stop_queue(ndev);
			...
			// while(0 == (canfd_ioread8(priv->reg_base+CANFD_RTIF_OFFSET) & CAN_FD_SET_TPIF_MASK));
			// //clr intr flag
			// canfd_reigister_set_bit(priv,CANFD_RTIF_OFFSET,CAN_FD_SET_TPIF_MASK);

			/* 2022 update delete this state */
			// PDEBUG( "Transmission status is %d. TX ends\n",canfd_ioread8(priv->reg_base+CANFD_CFG_STAT_OFFSET)&CAN_FD_TASCTIVE_MASK>>1);
...
static enum can_state get_of_chip_status(struct net_device *ndev){
	...
	if(can_stat & CAN_FD_SET_BUSOFF_MASK) return CAN_STATE_BUS_OFF;
	if((eir & CAN_FD_SET_EPASS_MASK) && ~(can_stat & CAN_FD_SET_BUSOFF_MASK)) return CAN_STATE_ERROR_PASSIVE;
	if(eir & CAN_FD_SET_EWARN_MASK && ~(eir & CAN_FD_SET_EPASS_MASK)) return CAN_STATE_ERROR_WARNING;
	if(~(eir & CAN_FD_SET_EPASS_MASK)) return CAN_STATE_ERROR_ACTIVE;
}
