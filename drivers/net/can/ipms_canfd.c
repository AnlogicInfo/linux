...
enum canfd_device_reg {
	/*Receive Buffer Registers 0x00-0x4f(not finished)*/
	CANFD_RUBF_OFFSET		=	0x00,
	CANFD_RUBF_ID_OFFSET		=	0x00,
	CANFD_RBUF_CTL_OFFSET		=	0x04,
	CANFD_RBUF_DATA_OFFSET		=	0x08,
	/*Transmit Buffer Registers 0x50-0x97(not finished)*/
	CANFD_TBUF_OFFSET		=	0x50,
	CANFD_TBUF_ID_OFFSET		=	0x50,
	CANFD_TBUF_CTL_OFFSET		=	0x54,
	CANFD_TBUF_DATA_OFFSET		=	0x58,

	/*Transmission Time Stamp 0x98-0x9f(not finished)*/
	CANFD_TTS_OFFSET		=	0x98,
...
enum canfd_reg_bitchange {
	...
	/*set TTTBM as 0->separate PTB and STB mode*/
	CAN_FD_OFF_FULLCAN_MASK		=	0xef,
...
static int canfd_device_driver_bittime_configuration(struct net_device *ndev)
{
	...
	if (priv->can_or_canfd == CANFD_MASK) {
		...
		canfd_iowrite8((0x1 << 7) | (dbt->phase_seg1 + dbt->prop_seg + 1),
			       priv->reg_base + CANFD_TDC_OFFSET);
...
static int can_rx(struct net_device *ndev)
{
	...
	if (rx_status & CAN_FD_RSTAT_NOT_EMPTY_MASK) {
		...
		PDEBUG("DLC in %s is %02x", __func__, dlc);
		 ...
		 * Then reset receive interrupt flag(we can not reset interrupt flag here)
		 */
...
static int canfd_rx(struct net_device *ndev)
{
	...
	/*since here we start canfd_rx interrupt*/
	PDEBUG("start %s interrupt\n", __func__);
	...
	if (rx_status & CAN_FD_RSTAT_NOT_EMPTY_MASK) {
		...
		if (control & CAN_FD_SET_EDL_MASK) {
			...
			if (!(control & CAN_FD_SET_RTR_MASK)) {
				*((u32 *)(cf->data + 0)) =
				priv->read_reg(priv, CANFD_RBUF_DATA_OFFSET);
				*((u32 *)(cf->data + 4)) =
				priv->read_reg(priv, CANFD_RBUF_DATA_OFFSET + 4);
			}
		}
...
static int canfd_rx_poll(struct napi_struct *napi, int quota)
{
	...
	while ((rx_status & CAN_FD_RSTAT_NOT_EMPTY_MASK) && (work_done < quota)) {
		(control & CAN_FD_SET_EDL_MASK) ? (work_done += canfd_rx(ndev)) :
		(work_done += can_rx(ndev));
		rx_status = canfd_ioread8(priv->reg_base + CANFD_RCTRL_OFFSET);
		...
		if (priv->can_or_canfd == CANFD_MASK) {
			if (can_is_canfd_skb(skb)) {
				...
				PDEBUG("TCMD is 0x%02x after writing data into resiger\n",
				       canfd_ioread8(priv->reg_base + CANFD_TCMD_OFFSET));
				PDEBUG("Interrupt flag register is 0x%02x \
				       after writing data into resiger\n",
				       canfd_ioread8(priv->reg_base + CANFD_RTIF_OFFSET));
			} else {
				/* Transmit can 2.0 frame under canfd mode*/
				PDEBUG("CAN FD 2.0B\n");
				//set Extended Data  Length 0, force BRS to be set 0 ;
			...
			/* deal with can 2.0 frame, when the driver can only
			 * be compatible with can 2.0 transceiver
			 */
...
static void canfd_error_interrupt(struct net_device *ndev, u8 isr, u8 eir)
{
	...
	PDEBUG("canfd is %s the beginning\n", (can_stat & CAN_FD_SET_BUSOFF_MASK) ?
	       "busoff" : "not busoff");
	...
	if (can_stat & CAN_FD_SET_BUSOFF_MASK) {
		...
		/* Error Passive mode */
		/* EPASS is either 0 or 1. Hypothetically, when transceiver is in BUSOFF mode,
		 * bit EPASS stays unchanged, i.e. "1"
		 */
		...
		if (skb) {
			/* Bosch specification indicates that Error count value
			 * greater than 96 is heavily disturbed.
			 */
			/*EWL=11*/
			cf->can_id |= CAN_ERR_CRTL;
...
static irqreturn_t canfd_interrupt(int irq, void *dev_id)
{
	...
	if ((isr & CAN_FD_SET_EIF_MASK) | (eir & (CAN_FD_SET_EPIF_MASK | CAN_FD_SET_BEIF_MASK))) {
		...
		/* reset EPIF and BEIF. Reset EIF. What if during error interrupt,
		 * new interrupt appears?
		 */
		canfd_reigister_set_bit(priv, CANFD_ERRINT_OFFSET, eir &
					(CAN_FD_SET_EPIF_MASK | CAN_FD_SET_BEIF_MASK));
...
static int canfd_chip_start(struct net_device *ndev)
{
	...
	/*set S_seg_1 S_Seg_2 S_SJW S_PRESC */
	temp = canfd_device_driver_bittime_configuration(ndev);
	if (temp < 0) {
...
static int canfd_driver_open(struct net_device *ndev)
{
	...
	PDEBUG("slow bit rate setting is phase_seg1 0x%08x phase_seg2 0x%08x \
			sjw 0x%08x prescaler 0x%02x prop_seg is 0x%02x\n",
			bt->phase_seg1, bt->phase_seg2, bt->sjw, bt->brp, bt->prop_seg);
	PDEBUG("fast bit rate setting is phase_seg1 0x%08x phase_seg2 0x%08x \
			sjw 0x%08x prescaler 0x%02x prop_seg is 0x%02x\n",
...
static const struct ethtool_ops canfd_ethtool_ops = {
...
static int canfd_driver_probe(struct platform_device *pdev)
{
	...
	/* ioremap: project the physical address(registers' addresses) to memory.
	 * value is the virtual address in mem.
	 */
	...
	ndev->ethtool_ops = &canfd_ethtool_ops;

	netif_napi_add_weight(ndev, &priv->napi, canfd_rx_poll, 16);//test budget=16
