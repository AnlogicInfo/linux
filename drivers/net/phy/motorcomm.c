 ...
 *
 * Copyright (C) 2012 - 2022 FMSH
 *
 * Driver for Motorcomm PHYs for FMSH PSOC platform.
 * Meng Fanqiang<mengfanqiang@fmsh.com.cn>
 *
 * Add device tree analysis part to support forcing phy connect mode and tx clk delay setting in device tree file.
 * to force phy connect mode and set tx clock delay, you must add the node blew in your device tree.
 * for YT8521:
 * xxxgmac0 {
 *     status = "okay";
...
static int yt8511_config_init(struct phy_device *phydev)
{
    ...
    if(of_property_read_u32(of_node, "yt,phy-delay", &phydelay) == 0)
    {
        ytphy_write_ext(phydev, 0x0c,phydelay);
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, phy-delay is set,delay-para is 0x%x\n",
            __func__, phydev->addr, phydelay);
#else
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, phy-delay is set,delay-para is 0x%x\n",
            __func__, phydev->mdio.addr, phydelay);
#endif
    }
    else
    {
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, warning:yt,phy-delay is invalid\n",
...
static int yt8521_config_init(struct phy_device *phydev)
{
    ...
    if(strcmp(ctype, "rgmii-id") == 0)
    {
        val = ytphy_read_ext(phydev, 0xa001);
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, read reg 0xa001 = 0x%x\n",
    ...
    {
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, phy-connection-type is invalid\n",
            __func__, phydev->addr);
#else
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, phy-connection-type is invalid\n",
            __func__, phydev->mdio.addr);
    ...
    if(of_property_read_u32(of_node, "yt,phy-delay", &phydelay) == 0)
    {
        ytphy_write_ext(phydev, 0xa003,phydelay);
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
        netdev_info(phydev->attached_dev, "%s , phy addr: %d, phy-delay is set,delay-para is 0x%x\n",
            __func__, phydev->addr, phydelay);
#else
